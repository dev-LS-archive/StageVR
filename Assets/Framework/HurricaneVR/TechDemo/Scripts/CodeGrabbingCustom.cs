...
namespace Framework.HurricaneVR.TechDemo.Scripts
{
    public class CodeGrabbingCustom : MonoBehaviour
    {
        public HVRHandGrabber Grabber { get; set; }
        public HVRHandGrabber Grabber2 { get; set; }
        public HVRHandGrabber GrabberSet;
        public HVRHandGrabber GrabberSet2;
        public HVRGrabbable Grabbable;
        public HVRGrabTrigger GrabTrigger;
        public HVRPosableGrabPoint GrabPoint;
        public HVRPosableGrabPoint GrabPoint2;
        ...
        public void Grab()
        {
            ...
            if (Grabbable && Grabber2)
            {
                if (GrabTrigger == HVRGrabTrigger.ManualRelease && Grabber2.GrabbedTarget == Grabbable)
                {
                    Grabber2.ForceRelease();
                    return;
                }
                
                //grabber needs to have it's release sequence completed if it's holding something
                if(Grabber2.IsGrabbing)
                    Grabber2.ForceRelease();
                
                Grabber2.Grab(Grabbable, GrabTrigger, GrabPoint2);
            }
        }
