...
float4 blend_draw(float2 uv, float4 color, float factor)
{
	...
	float alpha = min(color.w * factor, 1);
#if defined(FF_FLUID)
	return float4(old.xyz * (1 - alpha) + color.xyz * alpha, old.w + color.w * factor);
#elif defined(FF_NORMAL)
	return lerp(old, color, clamp(factor, 0, 1));
#else
	return old * (1 - alpha) + color * alpha;
#endif
}
